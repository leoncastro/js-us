 ...
 onready(function(){
  if( document.location.href.match(/\/embed\//) || $('#realdl>a') )
  {
   ...
   $('#mediaspace_wrapper').prepend( $('<div/>').attr('id', 'realdl')
                                    ...
                                    .append( $('<a/>').attr('href', '').attr('style', 'color: #fff; text-decoration: none;').text('FREE DOWNLOAD') ) );
   $('#realdl').show();
   var tmrstreamurl = setInterval(function(){
    if( $('#streamuri').text() != 'HERE IS THE LINK' )
    {
     $('#realdl a').attr('href', '/stream/' + $('#streamuri').text());
     $('#videooverlay').click();
