...
// @include      /^(https?:)?\/\/(openload\.(co|pw)|oload\.(cc|cloud|club|download|info|fun|live|site|space|stream|tv|win)|oladblock\.(me|services|xyz))\/*.*/
 ...
 onready(function(){
  if( document.location.href.match(/\/embed\//) || $('#realdl>a') )
  {
   ...
   var streamurl_tmr = setInterval(function(){
    ...
    if( streamurl_src )
    {
     var streamurl_url = location.origin + '/stream/' + /*TEMP_FIX: streamurl_src + */$('#lqEH1').text();
